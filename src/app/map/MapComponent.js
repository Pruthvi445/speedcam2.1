...
// Updated radarIcon for red light cameras (traffic signal colors, no speed limit number)
...
export default function SpeedcamHUD() {
  ...
  const [odo, setOdo] = useState(0); // local odometer, will sync with redux
  ...
  const crossedSet = useRef(new Set()); // track crossed camera IDs
  ...
  useEffect(() => {
    const initAuth = async () => {
      try {
        ...
        if (result?.user) {
          userId.current = result.user.uid;
          setAuthReady(true);
        }
      } catch (err) {
        console.error('Auth init failed:', err);
        userId.current = 'user-' + Date.now();
        setAuthReady(true);
      }
    };
    initAuth();
    ...
    const watchId = navigator.geolocation.watchPosition((pos) => {
      ...
      if (!firstFix && lastPos.current) {
        ...
        if (d > 0.1) {
          const newOdo = odo + d;
          setOdo(newOdo);
          dispatch(setTripDistance(newOdo)); // sync with redux
        }
      }
      firstFix = false;
      ...
      // Filter cameras within 100 meters (for speech)
      ...
      // Check for crossed cameras (distance < 50m and not yet crossed)
      ...
      // Speed alerts only for speed cameras
      ...
      // Update camera logic in redux (for relevant cameras, overspeed alert)
  ...
  const handleSearch = useCallback(async (e) => {
    e.preventDefault();
    const KEY = process.env.NEXT_PUBLIC_ORS_API_KEY; // use env var
    if (!KEY) {
    ...
    try {
      ...
      setRoute(path);
      // dispatch(setRelevant({ path, getDist })); // if you need to update relevant based on route
    } catch (err) {
      console.error(err);
  ...
  const handleAddCamera = useCallback(async (e) => {
    ...
    try {
      ...
      await SecureDataService.submitCameraRequest(cameraData, userId.current);
  ...
  const handleReport = async () => {
    if (!reportingCamera || !userId.current) {
      console.error('Missing reportingCamera or userId');
      return;
    }
    ...
    const success = await SecureDataService.reportCamera(reportingCamera.id, userId.current, reportReason);
    if (success) {
  ...
  return (
    <div className="relative h-[100dvh] w-screen bg-black text-white font-sans overflow-hidden">
      ...
      {/* 3-dot Menu */}
      {showMenu && (
        <div className="absolute top-16 right-3 z-[1001] bg-black/95 border border-white/10 rounded-xl p-2 w-48 backdrop-blur-xl">
        ...
        <div className="overflow-y-auto h-full px-4 pb-4">
          {activeTab === 'add' && !submitSuccess ? (
            ...
            <div className="py-2">
              ...
              <button onClick={handleAddCamera} disabled={submitLoading || !authReady || !tempPin} className="w-full bg-cyan-600 py-4 rounded-xl text-black font-black uppercase text-lg disabled:opacity-50 disabled:cursor-not-allowed">
      ...
      <style jsx global>{`
        ...
        /* Traffic light dots for red camera */
